    ...
    <div className="flex flex-col gap-6 xl:gap-8">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2 mb-6 xl:mb-8">
          ...
          {!isEditing && (
            <button onClick={onToggleEdit} className="p-1.5 text-slate-500 hover:text-white transition-colors" title="Editar detalles">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            {(aiData.genres || []).map(g => (
              <span key={g} className="px-2.5 py-1 bg-slate-800 border border-slate-700 rounded-lg text-slate-300 text-xs flex items-center gap-1.5">
                {g}
                {isEditing && (
                  <button onClick={() => onRemoveGenre(g)} className="text-slate-500 hover:text-red-400">
                    <X className="w-3 h-3" />
                  </button>
                )}
              </span>
          ...
          {isEditing && (
            <div className="flex gap-2">
              ...
              />
              <button onClick={onAddGenre} className="bg-slate-800 hover:bg-[rgb(var(--card-rgb))] p-1.5 rounded-lg border border-slate-700 text-white transition-colors">
          ...
          <div className="flex flex-col gap-2">
            {tracking.customLinks && tracking.customLinks.map((link) => (
              <div key={link.id} className="flex items-center justify-between group bg-slate-900/50 p-2 rounded-xl border border-slate-800/50">
                ...
                <button onClick={() => onRemoveCustomLink(link.id)} className="p-1.5 text-slate-600 hover:text-red-500 transition-colors">
                  <X className="w-3 h-3" />
                </button>
              </div>
            ))}
            <div className="flex gap-2 mt-2">
              ...
              />
              <button onClick={onAddCustomLink} className="bg-slate-800 hover:bg-[rgb(var(--card-rgb))] p-2 rounded-xl border border-slate-700 text-white transition-colors">
                <Plus className="w-4 h-4" />
              </button>
            </div>
          </div>
