...
export const CompactMediaCard: React.FC<CompactMediaCardProps> = React.memo(({ item, onClick, onIncrement, onToggleFavorite, onDelete }) => {
  ...
  return (
    <div
      ...
      className={`group relative rounded-xl overflow-hidden shadow-xl cursor-pointer transition-all duration-300 ease-out hover:scale-[1.02] hover:shadow-2xl flex flex-col bg-[#1A1D26] w-full ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
        }`}
      style={{
    ...
    >
      ...
      {showQuickAction && (
        <button
          ...
          className={`absolute bottom-20 right-3 z-40 p-2 md:p-3 rounded-full shadow-xl transition-all transform hover:scale-110 active:scale-95 border border-white/20 opacity-100 md:opacity-0 md:group-hover:opacity-100 ${isCompleteSeason ? 'bg-green-500 text-white' : 'bg-white text-slate-900'
            }`}
