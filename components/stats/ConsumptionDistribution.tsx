        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <span className="text-[10px] font-extrabold uppercase text-zinc-400 flex items-center gap-2" style={{ letterSpacing: '0.1em' }}>
              <PieChart className="w-4 h-4 text-emerald-500" style={{ filter: 'drop-shadow(0 0 6px #10b981)' }} /> Distribución de Consumo
            </span>
          ...
          <div className="grid grid-cols-3 gap-1 bg-[#09090B] p-1.5 rounded-xl w-full md:w-auto ring-1 ring-white/[0.06]">
            <button
              ...
              className={`px-2 py-1.5 rounded-md text-[10px] md:text-xs font-bold transition-all flex items-center justify-center gap-1 whitespace-nowrap w-full ${distributionAxis === 'genre' ? 'bg-emerald-500 text-white' : 'text-zinc-400 hover:text-white'}`}
              style={distributionAxis === 'genre' ? { boxShadow: '0 0 16px rgba(16, 185, 129, 0.5)' } : undefined}
            >
              <Hash className="w-3 h-3 hidden sm:inline" /> Géneros
            </button>
            <button
              onClick={() => setDistributionAxis('emotion')}
              className={`px-2 py-1.5 rounded-md text-[10px] md:text-xs font-bold transition-all flex items-center justify-center gap-1 whitespace-nowrap w-full ${distributionAxis === 'emotion' ? 'bg-emerald-500 text-white' : 'text-zinc-400 hover:text-white'}`}
              style={distributionAxis === 'emotion' ? { boxShadow: '0 0 16px rgba(16, 185, 129, 0.5)' } : undefined}
            >
              <Heart className="w-3 h-3 hidden sm:inline" /> Emociones
            </button>
            <button
              onClick={() => setDistributionAxis('platform')}
              className={`px-2 py-1.5 rounded-lg text-[10px] md:text-xs font-bold transition-all flex items-center justify-center gap-1 whitespace-nowrap w-full ${distributionAxis === 'platform' ? 'bg-white text-[#09090B]' : 'text-zinc-400 hover:text-white'}`}
              style={distributionAxis === 'platform' ? { boxShadow: '0 0 16px rgba(255, 255, 255, 0.25)' } : undefined}
            >
          ...
          <div className="animate-fade-in w-full">
            ...
            <div className="mb-8">
              <div className="flex justify-between text-xs text-zinc-400 mb-2 font-medium">
                <span className="flex items-center gap-1 min-w-0 truncate mr-2">
                  <Globe className="w-3 h-3 text-blue-400 flex-shrink-0" style={{ filter: 'drop-shadow(0 0 4px currentColor)' }} />
                  <span className="truncate">Streaming / Web ({linkPercent.toFixed(0)}%)</span>
                </span>
                <span className="flex items-center gap-1 min-w-0 truncate">
                  <HardDrive className="w-3 h-3 text-violet-400 flex-shrink-0" style={{ filter: 'drop-shadow(0 0 4px currentColor)' }} />
                  <span className="truncate">Local / Físico ({localPercent.toFixed(0)}%)</span>
                </span>
              </div>
            ...
            {stats.topPlatforms.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {stats.topPlatforms.map((p) => (
                  <div
                    key={p.domain}
                    className="flex items-center justify-between p-3 bg-[#1C1C1F] rounded-xl ring-1 ring-white/[0.06] hover:bg-white/[0.04] hover:ring-white/[0.12] transition-all"
                    style={{ borderLeft: '2px solid #3b82f640' }}
                  >
